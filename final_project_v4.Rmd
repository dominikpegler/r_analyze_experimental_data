...
data_correct <- data[ (answerCorrect == "correct") ]
wrongAnswers_percentage <- (1 - nrow(data_correct) / nrow(data)) * 100

# exclude trials with rt below 0.15s or above 1s
data_trimmed_correct <- data_correct[! (reactionTime < 0.150 | reactionTime > 1)  ] 
excluded_percentage <- (1 - nrow(data_trimmed_correct) / nrow(data_correct)) * 100
...
```{r validity effect, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r count of correct trials, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r valAnalysis, message=FALSE, warning=FALSE, include=FALSE}
...
ggplot(meanVal, aes(cueType, the.emmean)) +
   ...
   geom_line(data = data_validity, aes(cueType, validityEffect,
                                       group = sbj),
